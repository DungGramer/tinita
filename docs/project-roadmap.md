...
`truncateFileName` đã **tách thành 2 hàm** - `truncateFileName` (trả string) và
`truncateFileNameParts` (trả parts), mỗi hàm một file và một subpath export. Bỏ hẳn cờ
`output: 'string' | 'parts'` cùng 2 overload. Lý do ghi ở `code-standards.md` mục
"Quy Tắc API: Một Hàm, Một Kiểu Trả Về" - đây là **breaking change** với `tinita@0.0.1` đã publish.
...
| 17  | CSS ship rò rỉ ra global scope của host app - **sự cố production đã xảy ra thật**. **Đo lại trên artifact 2026-09-25** (bảng cũ suy từ source và SAI): rò rỉ thật là `body` background/color bị đè, class `.animate-fade-in` + `.transition-fast` của consumer bị CHIẾM bởi class cùng tên của library, và `Ping` vỡ layout khi host không có Tailwind. Rò rỉ 22 token KHÔNG tồn tại (`@theme inline` không được emit vào bundle). Quyết định thắng/thua là **thứ tự khai layer**, không phải specificity - consumer khai `@layer theme, base, components, utilities` thì tự bảo vệ được. Chi tiết + cách tái lập: `system-architecture.md` mục "Bề Mặt Rò Rỉ CSS", và `node compatibility/run.mjs l2 --no-pack` | `packages/tinita-react/src/styles/`, `dist/styles.css`                           |
...
- ~~Đưa 22 token trong `@theme inline` ra khỏi namespace Tailwind~~ **KHÔNG CẦN** - đo 2026-09-25:
  khối `@theme inline` không được emit vào `dist/styles.css` (0 match `--color-primary`). Việc này
  đã bị xoá khỏi phạm vi M1.
- **Thêm vào M1:** tài liệu hoá cho người dùng rằng khai `@layer theme, base, components, utilities`
  ở phía consumer là tự bảo vệ được - đo được là consumer thắng trong trường hợp đó. Rẻ hơn mọi
  thay đổi phía library.
