...
- tsup for TypeScript bundling:
  - tinita: `bundle: true` + một entry cho mỗi utility (tree-shaking theo subpath)
...
- Một entry tsup cho mỗi utility; `bundle: true` (bắt buộc, xem `system-architecture.md`)
...
**Impact**: High
**Likelihood**: Low
**Mitigation**: Automated testing of bundle sizes, một entry cho mỗi utility, và kiểm mọi đường dẫn `exports` resolve được sau build
