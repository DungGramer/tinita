import type { DownloadInput, DownloadOptions, DownloadResult } from '../types/download-types';
import { DownloadError, DownloadErrorCode } from '../errors/download-errors';
import { detectInputType } from '../core/detect-input-type';
import { toBlob } from '../core/to-blob';
...
export async function download(
  ...
  try {
    ...
    // Strip onError to avoid double-fire (download() handles it in outer catch)
    if (inputType === 'url-string' || inputType === 'url-object') {
      const { onError: _skipOnError, onBeforeDownload: _skipBefore, onAfterDownload: _skipAfter, ...urlOptions } = options;
      const task = downloadFromUrl(input as string | URL, urlOptions);
