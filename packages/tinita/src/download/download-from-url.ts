import type { DownloadFromUrlOptions, DownloadTask, DownloadResult, DownloadProgress } from './types';
import { HttpStatusError, NetworkError, TimeoutError, AbortDownloadError, InvalidBlobResponseError } from './errors';
...
import { resolveRetryConfig, withRetry } from './retry-download';
import { waitMinDuration } from './min-duration-delay';
import { createSpeedSmoother } from './smooth-progress';
import { parseRetryAfterMs } from './is-retryable-error';
...
export function downloadFromUrl(
  ...
  function createAttempt(attempt: number): Promise<DownloadResult> {
    ...
    return new Promise<DownloadResult>((resolve, reject) => {
      ...
      xhr.onload = () => {
        ...
        if (minDuration > 0) {
          const startTime = Date.now();
          waitMinDuration(startTime, minDuration, () => aborted)
            .then(() => {
  ...
  const resultPromise: Promise<DownloadResult> = retryOpts
